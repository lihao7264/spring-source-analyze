...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * 默认scope值，bean的作用范围
	 * <p>
	 * 默认作用域名称的常量：""，等同于单例状态，除非从父bean定义中覆盖（如果适用）。
	 */
	 ...
	 * 返回解析的自动代码（将AUTOWIRE_AUTODETECT解析为AUTOWIRE_CONSTRUCTOR或AUTOWIRE_BY_TYPE）。
	 * @see #AUTOWIRE_AUTODETECT
	 * @see #AUTOWIRE_CONSTRUCTOR
	 * @see #AUTOWIRE_BY_TYPE
	 */
	public int getResolvedAutowireMode() {
		//如果是AUTOWIRE_AUTODETECT的自动装配方式
		if (this.autowireMode == AUTOWIRE_AUTODETECT) {
			// Work out whether to apply setter autowiring or constructor autowiring.
			// If it has a no-arg constructor it's deemed to be setter autowiring,
			// otherwise we'll try constructor autowiring.
			Constructor<?>[] constructors = getBeanClass().getConstructors();
			//构造函数中存在无参构造的话，就返回AUTOWIRE_BY_TYPE
			for (Constructor<?> constructor : constructors) {
				if (constructor.getParameterCount() == 0) {
					return AUTOWIRE_BY_TYPE;
				}
			}
			//不存在无参构造的话，就返回AUTOWIRE_CONSTRUCTOR
